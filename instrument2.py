...
class Apparatus:
    ...
    def line_detect_possible_demo(self, image, center, tg):
        ...
        if a * b < 0 and max(abs(a), abs(b)) > np.pi / 4:
            ...
            self.angle.append(math.degrees(np.pi / 2 - (a + b) / 2)) if res[angle1[1]][1] == 1 else self.angle.append(
        ...
        if a * b < 0 and max(abs(a), abs(b)) > np.pi / 4:
            ...
            self.angle.append(math.degrees(np.pi / 2 - (a + b) / 2)) if res[angle2[1]][1] == 1 else self.angle.append(
                math.degrees(np.pi / 2 - (a + b) / 2) + 180)
...
if __name__ == '__main__':
    apparatus = Apparatus(r'D:/1/yi2.png')
    # 读取图片
    apparatus.test()
    k = cv.waitKey(0)
    if k == 27:
